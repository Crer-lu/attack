...
if os.path.exists("weights/adversarial_image_black_class_condition/noise_simulator.ckpt"):
    noise_simulator.load_state_dict(torch.load("weights/adversarial_image_black_class_condition/noise_simulator.ckpt"))

if os.path.exists("weights/adversarial_image_black_class_condition/latent_classifier.ckpt"):
    latent_classifier.load_state_dict(torch.load("weights/adversarial_image_black_class_condition/latent_classifier.ckpt"))
...
for iteration in progress:
    ...
    if iteration % SAVE_DELTA==SAVE_DELTA - 1:
        torch.save(noise_simulator.state_dict(),f"weights/adversarial_image_black_class_condition/noise_simulator_{iteration}.ckpt")
        torch.save(noise_simulator.state_dict(),f"weights/adversarial_image_black_class_condition/noise_simulator.ckpt")
        ...
        for i in range(real.shape[0]):
            ...
            plt.imshow(fake[i])
        plt.savefig(f"tests/{iteration}_({int(target_real[0])}_{int(target_real[1])}_{int(target_real[2])}_{int(target_real[3])})_({int(target_fake[0])}_{int(target_fake[1])}_{int(target_fake[2])}_{int(target_fake[3])})_({int(target_cond[0])}_{int(target_cond[1])}_{int(target_cond[2])}_{int(target_cond[3])}).png")