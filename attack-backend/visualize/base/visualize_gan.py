...
discriminator_n_losses = []
discriminator_c_losses = []
generator_c2n_losses = []
generator_n2c_losses = []
...
for iteration in progress:
    ...
    discriminator_n_losses.append(numpy.array(discriminator_n_loss).mean())
    discriminator_c_losses.append(numpy.array(discriminator_c_loss).mean())
    generator_c2n_losses.append(numpy.array(generator_c2n_loss).mean())
    generator_n2c_losses.append(numpy.array(generator_n2c_loss).mean())

    if iteration % SAVE_DELTA == SAVE_DELTA - 1:
        np_discriminator_n_losses = numpy.array(discriminator_n_losses)
        np_discriminator_c_losses = numpy.array(discriminator_c_losses)
        np_generator_c2n_losses = numpy.array(generator_c2n_losses)
        np_generator_n2c_losses = numpy.array(generator_n2c_losses)
        x = numpy.arange(np_discriminator_n_losses.shape[0])
        plt.clf()
        plt.plot(x,np_discriminator_n_losses)
        plt.xlabel("Iteration")
        plt.ylabel("Loss")
        plt.show()
        plt.clf()
        plt.plot(x,np_discriminator_c_losses)
        plt.xlabel("Iteration")
        plt.ylabel("Loss")
        plt.show()
        plt.clf()
        plt.plot(x,np_generator_c2n_losses)
        plt.xlabel("Iteration")
        plt.ylabel("Loss")
        plt.show()
        plt.clf()
        plt.plot(x,np_generator_n2c_losses)
        plt.xlabel("Iteration")
        plt.ylabel("Loss")
        plt.show()